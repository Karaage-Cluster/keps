...
* Try to figure out the purpose of the quantity field. The quantity field
  cannot be NULL, and doesn't appear to make sense for CPU Hours.
...
The ``AllocationPool`` defines a set of ``Allocations`` and maps them to a
``ResourcePool``.
...
* Why not have ``Allocation`` point directly to a ``ResourcePool`` rather then
  indirectly via the ``AllocationPool``? Is the ``AllocationPool`` really
  required?
...
* ``range_start`` and ``range_end`` are ``DateTime`` not just ``Date``.
...
* Should ``content_type`` get moved to the ``ResourcePool`` or ``Resource``
  models?
...
* These attributes don't need to be part of the ``unique_together`` attribute
  ...
  * ``person`` - as the ``Person`` associated with an ``Account`` will never
    change.
  * ``resource_pool`` - calculated from other values.
  * ``resource`` - calculated from other values.
  * ``scheme`` - calculated from other values.
  * ``allocation_pool``  - calculated from other values.
  * ``allocation_period``  - calculated from other values.
...
* We cannot automatically generate a ``scheme``, ``allocation_pool`` and
  ...
  installs.
* Do we really want to assign the record to an ``allocation_pool`` when
  recorded? If there are multiple ``allocation_pools`` that are valid, which
  one should we use? Or would it be better to assign it to ``allocation_pools``
  later when displaying the data?
* Recording the ``scheme`` here makes no sense. An ``AllocationPool`` can have
  multiple ``Allocations``, each ``Allocation`` from a different ``Grant``.
  Each ``Grant`` can have a different ``Scheme``. We can only record one
  ``scheme`` however.
...
The ``ResourcePools`` can have two different sets of properties. e.g. CPU hours
is accumulative and each new entry will add to the previous total. Where as
with disk space, we are more concerned about the maximum about of disk space
used at any one time. There should perhaps be an extra field added to
distinguish the two different cases.

There should also be a property that defines the units used by this field.

CPUJob model
~~~~~~~~~~~~
Once the `Usage model`_ is finalized, the fields in the ``CPUJob`` model
need to be revised.
