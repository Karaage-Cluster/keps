...
* Maybe these attributes should be part of the ``unique_together``
  attribute????
...
* Should these attributes should be part of the ``unique_together`` attribute?
...
* The ``scheme`` field should get removed. We cannot automatically generate a
  ``scheme``, ``allocation_pool`` and ``allocation_period`` objects on upgrade
  that would make sense for all installs.
